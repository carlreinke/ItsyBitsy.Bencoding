...
namespace ItsyBitsy.Bencoding.Tests
{
    ...
    {
        [Fact]
        public static void ReadTokenType_DefaultInstance_ThrowsInvalidBencodeExceptionAndGoesToErrorState()
        {
            BencodeSpanReader reader = default;

            var ex = AssertThrows<InvalidBencodeException>(ref reader, (ref BencodeSpanReader r) => _ = r.ReadTokenType());
            int position = reader.Position;
            var errorTokenType = reader.ReadTokenType();

            Assert.Equal("Expected 'd', 'i', 'l', or '0'-'9' but reached the end of the source buffer while reading the token type.", ex.Message);
